...
export interface IconProps extends SVGProps<SVGSVGElement> {
  ...
  active?: boolean
  /** Prefer filled path when available (e.g. bottom nav solid glyphs) */
  forceFilled?: boolean
  className?: string
}
...
type IconGlyph = {
  outline: string
  filled?: string
  filledFillRule?: 'evenodd'
}

const paths: Record<IconName, IconGlyph> = {
  home: {
    outline:
      'M2.25 12L11.204 3.045a1.125 1.125 0 011.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25',
    filled:
  ...
  folder: {
    outline:
      'M2.25 12.75V12A2.25 2.25 0 014.5 9.75h15A2.25 2.25 0 0121.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44Z',
    filled:
  ...
  'plus-circle': {
    ...
    filled:
      'M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 9a.75.75 0 00-1.5 0v2.25H9a.75.75 0 000 1.5h2.25V15a.75.75 0 001.5 0v-2.25H15a.75.75 0 000-1.5h-2.25V9Z',
    filledFillRule: 'evenodd',
  },
  search: {
...
export function Icon({
  ...
  const { outline, filled, filledFillRule } = paths[name]
  const useFilled = Boolean(filled) && (forceFilled || active)
  const d = useFilled ? filled! : outline
  const isFilledVariant = useFilled
  const pathFillRule = isFilledVariant && filledFillRule === 'evenodd' ? 'evenodd' : undefined
  ...
  return (
    <svg
      ...
      {...svgProps}
    >
      <path d={d} fillRule={pathFillRule} clipRule={pathFillRule} />
    </svg>
  )
}
